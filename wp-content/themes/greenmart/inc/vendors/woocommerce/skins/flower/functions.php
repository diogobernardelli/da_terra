...
if ( ! function_exists( 'greenmart_woo_show_product_loop_sale' ) ) {
    /*Change sales woo*/
    add_filter( 'woocommerce_sale_flash', 'greenmart_woo_show_product_loop_sale' );
    function greenmart_woo_show_product_loop_sale( $html ) {

        $product         = wc_get_product();
        $_product_sale   = $product->get_sale_price();
        $featured        = $product->is_featured();
        $return_content  = ($featured) ? '<span class="featured">'. esc_html__('hot', 'greenmart') .'</span>' : '';

        if(!empty($_product_sale ))  {
            $percentage_new = $product->get_regular_price() - $product->get_sale_price();
            $percentage_new = '<span class="saled"><span>' . esc_html__('Save', 'greenmart') . '</span>' . wc_price(trim( $percentage_new )) . '</span>';
            $percentage_new .= $return_content;

            $percentage_older = round( ( ( $product->get_regular_price() - $product->get_sale_price() ) / $product->get_regular_price() ) * 100 );
            $percentage_older = '-  ' . trim( $percentage_older ) . '%';
                     
        } else {
            $percentage_new = '<span class="saled text">'. esc_html__( 'Sale', 'greenmart' ) . '</span>';
            $percentage_new .= $return_content;

            $percentage_older = esc_html__( 'Sale', 'greenmart' );
        }

        return str_replace( $percentage_older, $percentage_new, $html );
    }
}
...
if(!function_exists('greenmart_woocommerce_price_html')){
    ...
    add_filter('woocommerce_get_price_html', 'greenmart_woocommerce_price_html', 100, 2);
}
