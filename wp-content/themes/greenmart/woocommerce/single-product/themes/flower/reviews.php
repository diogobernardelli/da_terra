					...
					<div id="review_form_wrapper" class="review_form_wrapper">
						<div id="review_form">
							<?php
								...
								$comment_form = array(
									'title_reply'          => have_comments() ? esc_html__( 'Leave you review', 'greenmart' ) : esc_html__( 'Be the first to review', 'greenmart' ) . ' &ldquo;' . get_the_title() . '&rdquo;',
									'title_reply_to'       => esc_html__( 'Leave a Reply to %s', 'greenmart' ),
									'comment_notes_before' => '',
									'comment_notes_after'  => '',
									'fields'               => array(
										'author' => '<p class="comment-form-author form-group">
													<label class="control-label">' . esc_html__( 'Name:', 'greenmart' ) .'</label>
										...
										'email'  => '<p class="comment-form-email form-group">
													<label class="control-label">' . esc_html__( 'Email:', 'greenmart' ) .'</label>
										            <input id="email" placeholder="'. esc_html('Enter your e-mail...', 'greenmart') .'" class="form-control" name="email" type="text" value="' . esc_attr(  $commenter['comment_author_email'] ) . '" size="30" aria-required="true" /></p>',
									),
								...
								if ( get_option( 'woocommerce_enable_review_rating' ) === 'yes' ) {
									...
									<label for="rating" class="control-label">' . esc_html__( 'Rating', 'greenmart' ) .'</label>
