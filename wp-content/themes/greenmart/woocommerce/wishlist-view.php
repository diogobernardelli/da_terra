        ...
        <div class="wishlist-title <?php echo ( $is_custom_list ) ? 'wishlist-title-with-form' : ''?>">
            ...
            <?php if( $is_custom_list ): ?>
                <a class="btn button show-title-form">
                    ...
                    <?php esc_html_e( 'Edit title', 'greenmart' ) ?>
                </a>
            <?php endif; ?>
        </div>
        <?php if( $is_custom_list ): ?>
            <div class="hidden-title-form">
                ...
                <button>
                    <?php echo apply_filters( 'yith_wcwl_save_wishlist_title_icon', '<i class="'. greenmart_get_icon('icon_wishlist') . '"></i>' )?>
                    <?php esc_html_e( 'Save', 'greenmart' )?>
                </button>
                <a class="hide-title-form btn button">
                    <?php echo apply_filters( 'yith_wcwl_cancel_wishlist_title_icon', '<i class="fa fa-remove"></i>' )?>
                    <?php esc_html_e( 'Cancel', 'greenmart' )?>
                </a>
            </div>
        <?php endif; ?>
    <?php
        ...
        if( count( $wishlist_items ) > 0 ) :
            ...
            foreach( $wishlist_items as $item ) :
                ...
                if( $product && $product->exists() ) :
	                ?>
                    <?php if ( wp_is_mobile() ) { ?>
                        <tr class="mobile" id="yith-wcwl-row-<?php echo esc_attr($item['prod_id']); ?>" data-row-id="<?php echo esc_attr($item['prod_id']); ?>">
                            <?php if( $show_cb ) : ?>
                            ...
                            <td class="product-info-mobile">
                                ...
                                <span class="price">
                                    <?php echo ($product->get_price()) ? $product->get_price_html() : apply_filters( 'yith_free_text', esc_html__( 'Free!', 'greenmart' ) ); ?>
                                </span>
                                <?php echo ($stock_status == 'out-of-stock') ? '<span class="wishlist-out-of-stock">' . esc_html__( 'Out of Stock', 'greenmart' ) . '</span>' : '<span class="wishlist-in-stock">' . esc_html__( 'In Stock', 'greenmart' ) . '</span>'; ?>
                                <!-- Date added -->
                                <?php
                                ...
                                <?php if( $is_user_owner && $repeat_remove_button ): ?>
                                    <a href="<?php echo esc_url( add_query_arg( 'remove_from_wishlist', $item['prod_id'] ) ) ?>" class="remove_from_wishlist button" title="<?php esc_html_e( 'Remove this product', 'greenmart' ) ?>"><?php esc_html_e( 'Remove', 'greenmart' ) ?></a>
                                <?php endif; ?>
                            </td>
                            ...
                            <td class="product-remove">
                                <div>
                                    <a href="<?php echo esc_url( add_query_arg( 'remove_from_wishlist', $item['prod_id'] ) ) ?>" class="remove remove_from_wishlist" title="<?php esc_html_e( 'Remove this product', 'greenmart' ) ?>"><i class="fa fa-trash-o" aria-hidden="true"></i></a>
                                </div>
                            </td>
	        ...
	        <td colspan="<?php echo esc_attr( $column_count ) ?>">
	            <?php if( $show_cb ) : ?>
		            <div class="custom-add-to-cart-button-cotaniner">
		                <a href="<?php echo esc_url( add_query_arg( array( 'wishlist_products_to_add_to_cart' => '', 'wishlist_token' => $wishlist_token ) ) ) ?>" class="button alt" id="custom_add_to_cart"><?php echo apply_filters( 'yith_wcwl_custom_add_to_cart_text', esc_html__( 'Add the selected products to the cart', 'greenmart' ) ) ?></a>
		            </div>
	            <?php endif; ?>
	            ...
	            <?php if ( $is_user_owner && $show_ask_estimate_button && $count > 0 ): ?>
		            <div class="ask-an-estimate-button-container">
	                    ...
	                    <?php echo apply_filters( 'yith_wcwl_ask_an_estimate_text', esc_html__( 'Ask for an estimate', 'greenmart' ) ) ?>
	                </a>
		            </div>
	            <?php endif; ?>
	...
	<div id="ask_an_estimate_popup">
		<form action="<?php echo esc_url($ask_estimate_url) ?>" method="post" class="wishlist-ask-an-estimate-popup">
			<?php if( ! is_user_logged_in() ): ?>
				<label for="reply_email"><?php echo apply_filters( 'yith_wcwl_ask_estimate_reply_mail_label', esc_html__( 'Your email', 'greenmart' ) ) ?></label>
